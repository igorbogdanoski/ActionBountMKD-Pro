...
## ✅ Phase 7 — Поправки и Полирање (Bugfix Sprint)

> **Commit:** `2026-06-07` | Верзија: 7.1.0
>
> Решени сите познати веб-бугови (W1–W5) и исчистен неупотребуван код.

### Поправени бугови

**W1** ✅ — „Уреди" копче / мртов Quick-Edit modal
Навигацијата до `/creator/:id` беше веќе поправена. Целосно отстранет мртвиот Quick-Edit modal (state, handlers, JSX ~70 линии). Отстранети неупотребувани imports (`X`, `Check`, `saveQuest`).
Фајлови: `BoundsDashboard.tsx`

**W3** ✅ — „Земи моја локација" во FindSpotEditor
Веќе беше целосно имплементирано: `getMyLocation`, map toggle, Haversine. Нема потреба од промена.
Фајлови: `FindSpotEditor.tsx`

**W4** ✅ — `isPublic` поле не се пишувало при зачувување
`saveQuest` сега пишува `isPublic: quest.visibility === 'public'`. Firestore правилото проверуваше и двете (`visibility == 'public' || isPublic == true`) — сега полето е конзистентно. Мобилната апликација може сигурно да прашува `where('isPublic', '==', true)`.
Фајлови: `utils/storage.ts`

**W5** ✅ — Терминот „квест" наместо „авантура"
Поправени i18n клучеви: `creator.questSettings` → „Поставки на Авантурата" / „Adventure Settings"; `creator.dangerZone.deleteQuest` → „Избриши авантура" / „Delete adventure".
Фајлови: `i18n/mk.json`, `i18n/en.json`

**W2** ⏳ — Нема email/лозинка логин (само Google OAuth)
Планирано за Phase 7.2 — бара Firebase `EmailAuthProvider` интеграција и нов UI.

### Статус на фазата

| Под-фаза | Статус |
| --- | --- |
| 7A Доверба/усогласеност | ✅ |
| 7B Почетна + SEO | ✅ |
| 7C Дизајн систем | ✅ |
| 7D Педагошки слој | ✅ |
| 7E Ученичко искуство | ✅ |
| 7F Play Store (AAB/eas.json) | ◐ бара store credentials |
| 7G SaaS зрелост (XLSX ✅, billing ⏳) | ◐ бара Cloud Functions |
| 7H Квалитет + Sentry + E2E | ✅ |
| 7.1 Bugfix Sprint | ✅ Завршена |

### Технички метрики (Phase 7.1)

- TypeScript грешки: **0** (строг mode)
- Отстранети мртви компоненти: Quick-Edit modal (~70 линии)
- Поправени Firestore конзистентност пропусти: `isPublic` сега секогаш синхронизиран со `visibility`
- i18n конзистентност: сите кориснички strings за „авантура" се унифицирани

---

**Последно ажурирање:** 2026-06-07 | Верзија: 7.1.0 — Phase 7 bugfix sprint завршен
