...
- Типографија: заоблен читлив фонт (Quicksand/Montserrat) + Pacifico само за бренд
- Tokens: бои, радиуси, сенки, spacing во Tailwind theme; еднаков dark-mode паритет
- Заедничка UI библиотека (Button, Card, Toggle, Badge, Modal) — реупотреба во creator + player + landing
- Векторски илустрации/иконографија конзистентни

**Фајлови:** `tailwind.config`/theme, `src/components/ui/*` (нов), refactor низ постоечките екрани
...
- Цели на учење + предмет/одделение/курикулум таг по авантура (полиња + филтри)
- Рубрики и насочена повратна информација за рачно оценети задачи (MISSION/SURVEY)
- Класови/групи: именувани групи ученици, доделување авантура, преглед по ученик
- Извоз на оцени (по ученик/етапа) + сертификати по класа
- Опц.: Google Classroom „доделено како задача"
- Библиотека на педагошки шаблони по предмет (надгради на постоечките seed шаблони)

**Фајлови:** `types.ts`, `QuestSettingsPanel.tsx`, `ResultsDashboard.tsx`, нови `groups`/`assignments` модели + rules
...
1. **7A** Доверба/усогласеност (Footer + Privacy + Terms) — во тек
2. **7B** Почетна + SEO длабочина
3. **7C** Дизајн систем
