...
| 2026-07-23 | H7-5 Objective mastery CSV извештај во ClassGroups | pending | PASS | 656/656 PASS; 2 focused ClassGroups contracts | PASS | `ClassGroups` „Оцени и сертификати" добива нов ред: избор на доделена авантура (само оние со дефинирани `learningObjectives`, инаку selector е disabled со јасна порака) и „Извези совладаност на цели (CSV)", кој ја повикува `buildObjectiveMasteryReport` директно (нема нова агрегациска логика) и генерира per-student % mastery колона по objective, со UTF-8 BOM исто како постоечкиот „Извези оцени" export. CSV е намерно избран наместо PDF — постоечкиот PDF механизам (сертификати) останува одделен; slash во roadmap ставката се толкува како „или". Desktop 1440×900 и mobile 375×812 преку QA harness (port 3100; `e2e/fixtures/qaStorage.ts` прошираен со `qa-objective-1`/`pedagogy.learningObjectives` за реален browser lifecycle) целосен create-group→add-student→assign→export lifecycle; zero console errors; production build непроменет. |

## Следна акција

Commit-ирај H7-5, потоа продолжи со H7-6: privacy/authorization review за сите нови H6/H7 извештаи (student results, gradebook, objective mastery) — потврди дека секој read/export патека е ограничена на `creatorId`/group `ownerId` на автентицираниот наставник (веќе е случај низ постоечкиот `storage.ts` API-слој, но направи експлицитен аудит наместо претпоставка), дека нема client-side рута што протекува податоци на ученик надвор од сопствената класа/квест, и дека Firestore rules го покриваат истото на серверска страна за секое ново поле додадено во H6/H7 (`studentId`, `attemptId`, `approvedAt/By`, `objectiveRef`, `learningObjectives`). Ова е последната отворена H7 ставка — по затворањето, H7 е целосно затворен. Не воведувај institution-specific curriculum taxonomy без експлицитен source-of-truth.
