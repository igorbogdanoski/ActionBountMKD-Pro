...
| R15 | Dependency advisories се менуваат со registry базата; свежиот audit на 2026-08-04 пријави 27 наоди (6 high, 20 moderate, 1 low) | Таргетирани non-force updates (`react-router-dom` 7.18.2, PostCSS 8.5.25 и безбедни транзитивни patches) го намалија свежиот резултат на 21 (2 high, 19 moderate, 0 critical/low). Преостанатите high се React Router RSC advisories што не се активни во client-only SPA патеката; moderate graph-от е Expo/Firebase Admin и бара upstream/major compatibility batch. Без downgrade или `audit fix --force` | Controlled |
...
| R18 | `isValidQuest`/`isValidResult` плитко валидираат само top-level полиња; nested optional полиња (`pedagogy.learningObjectives[].label`, `tags`, `learningGoals`) немаат server-side bounds | Client веќе ги наметнува вистинските граници (`MAX_LEARNING_GOALS`/`MAX_LEARNING_GOAL_LENGTH`); зависи од owner-only write гејт; длабока server-side validation е одделен, поширок hardening item, не специфичен за H7 | Controlled |
| R19 | Roster launch линкови дејствуваат како bearer token — секој со линкот може да поднесе резултат како тој `studentId` | Намерен tradeoff на no-login roster play (H6-2); документиран, не нов gap од H7 | Controlled |
...
| R22 | Локалната околина има `NODE_TLS_REJECT_UNAUTHORIZED=0`, што ја исклучува TLS certificate проверката за Node/npm | Не е repo промена; отстрани ја environment поставката во доверливото shell/CI ниво и повтори registry/deploy проверки со TLS verification | Open |
...
Commit-ирај го 2026-08-04 hardening batch по сопственичка потврда; push не е направен. **H7 (Objectives, coverage и mastery) останува целосно затворен**, а R1–R7 migration/coverage debt е re-audit-иран и затворен.

Следната фаза, H8 (Institutional SaaS completeness), е квалитативно поразлична од H4–H7: billing lifecycle, onboarding email секвенца, institutional документација и production monitoring/alerting/backup се business/infra одлуки (провајдер за billing, email сервис, monitoring стек), не чисто код hardening со самоочигледен опсег. Пред H8 implementation избери приоритет и провајдери/алатки. Независните следни hardening batches се: отстранување на локалниот TLS bypass (R22), deep Firestore bounds validation (R18), roster bearer-link threat-model/optional PIN (R19), и одделен Expo/Firebase Admin/React Router compatibility review за преостанатиот R15 dependency graph.
