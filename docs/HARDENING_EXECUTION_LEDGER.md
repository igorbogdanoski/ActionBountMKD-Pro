...
Продолжи со H6-6 closure audit: провери ги сите Firestore result queries против index барањата, изврши backward-compatibility matrix за legacy резултати без `studentId`/`attemptId`/approval metadata и затвори ги преостанатите duplicate-name, renamed-student, guest и multi-attempt тестови. Документирај дали е потребен index/rules migration; не деплој rules без експлицитна production дозвола. Потоа затвори H6 и продолжи со H7 stable objective reference model. R15 останува контролиран и се ревидира при Expo/Firebase Admin major-upgrade планирањето.
