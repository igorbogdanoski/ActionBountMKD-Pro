...
Продолжи со H6-4: дефинирај и имплементирај чист result-selection policy за `first`, `latest`, `best` и teacher-approved резултат, со јасна default политика за gradebook/certificates и backward compatibility за legacy records. Политиката прво мора да биде pure/shared и покриена за roster students, duplicate names, guests, tied scores и повеќе attempts; UI controls доаѓаат дури по стабилен policy contract. R15 останува контролиран и се ревидира при Expo/Firebase Admin major-upgrade планирањето.
