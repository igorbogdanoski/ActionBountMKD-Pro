...
Продолжи со H6-5: имплементирај owner-only teacher approval/revoke write contract и минимални ResultsDashboard controls што јасно го прикажуваат бројот на обид и approval state. Firestore update мора да дозволи само `approvedAt`/`approvedBy`, да го врзе `approvedBy` со `request.auth.uid` при approve и да дозволи безбедно revoke; resubmission останува append-only нов attempt. Покриј approve, revoke, unauthorized mutation и desktop/mobile UI state пред H6 rules/index/backward-compatibility closure. R15 останува контролиран и се ревидира при Expo/Firebase Admin major-upgrade планирањето.
